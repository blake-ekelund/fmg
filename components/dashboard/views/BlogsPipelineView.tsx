...
import type { BlogPipelineCounts } from "../hooks/useDashboardBlogPipeline";

const STAGES: { key: keyof BlogPipelineCounts; label: string; accent: string; bg: string }[] = [
...
type Props = {
  counts: BlogPipelineCounts;
  loading: boolean;
};

export default function BlogsPipelineView({ counts, loading }: Props) {
  if (loading) {
    return (
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between pt-2 border-t border-gray-100">
        ...
        <Link href="/blog-posts" className="text-xs text-blue-600 hover:text-blue-800 font-medium">
          View blog posts
        </Link>
      </div>
    </div>
