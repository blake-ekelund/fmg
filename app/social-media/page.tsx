...
import { Sparkles, Mail, X, Send, Hash } from "lucide-react";
...
import type { SocialPost, SocialPostStatus } from "@/components/social-media/types";
...
export default function SocialMediaPage() {
  ...
  const loadPosts = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .eq("platform", "Instagram / Facebook")
  ...
  const visible = useMemo(() => {
    if (brand === "all") return posts;
    return posts.filter((p) => p.brand === brand);
  }, [posts, brand]);
  ...
  async function handleDrop(newStatus: SocialPostStatus) {
    ...
    if (!currentPost || currentPost.status === newStatus) { setDraggingId(null); return; }
          ...
          >
            ...
            <div className="px-5 py-4 space-y-3">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 rounded-lg bg-gray-50 border border-gray-200 px-3 py-2 text-sm text-gray-700">
                  Social Post Ready — Instagram / Facebook • {emailPost.brand}
                </div>
              </div>
              <div>
                ...
                <div className="rounded-lg bg-gray-50 border border-gray-200 px-4 py-3 text-sm text-gray-700 leading-relaxed space-y-3">
                  <p>Hi Blake,</p>
                  <p>Please find the attached social media post ready for publish.</p>
                  <div className="rounded-lg bg-white border border-gray-200 px-3 py-2.5">
                    <div className="flex items-center gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[10px] text-gray-400">{emailPost.brand} • {emailPost.post_type}</div>
                      </div>
                    </div>
                  </div>
